...
def _parse_threshold_fragment(fragment: str) -> JsonDict | None:
    ...
    pattern = re.compile(
        ...
        rf"\s+(?:is\s+)?(?P<operator>{'|'.join(OPERATOR_WORDS)})\s+{VALUE_UNIT_PATTERN}",
        flags=re.IGNORECASE,
    )
    ...
    return {
        ...
        "transform": transform,
        "operator": OPERATOR_WORDS[match.group("operator").lower()],
...
def parse_range_conditions(text: str) -> List[JsonDict]:
    """Parse symbolic and natural-language vehicle-speed range predicates."""
...
def parse_threshold_conditions(text: str) -> List[JsonDict]:
    """Parse threshold predicates across the requirement text."""

    conditions: List[JsonDict] = []
    pattern = re.compile(
        rf"(?P<expr>(?:\|\{{[^}}]+\}}\||abs\{{[^}}]+\}}|absolute\s*\{{[^}}]+\}}|{SIGNAL_PATTERN})"
        rf"\s+(?:is\s+)?(?:{'|'.join(OPERATOR_WORDS)})\s+\d+(?:\.\d+)?\s*(?:kph|Nm))",
        flags=re.IGNORECASE,
    )
...
def parse_trend_dependencies(text: str) -> List[JsonDict]:
    ...
    for match in re.finditer(
        r"\bwith\s+(?P<trend>increasing|decreasing)\s+(?P<signal>Driver Torque|Column Torque|assist torque|torque demand|S_[A-Z0-9_]+)\b",
        text,
        flags=re.IGNORECASE,
    ):
...
def parse_conditions(text: str, normalized_entities: List[JsonDict] | None = None) -> List[JsonDict]:
    """Parse Layer4 condition objects from a requirement sentence."""
    ...
    conditions.extend(parse_trend_dependencies(text))
    if not conditions:
